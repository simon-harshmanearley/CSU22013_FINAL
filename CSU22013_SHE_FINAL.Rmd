...
count_commits_by_DevRepo <- function(commits_df){
  #return(aggregate(email ~ dev_name + repo + email, data = commits_df, FUN = length))
    ...
    return(aggregatedDataFrame <- aggregatedDataFrame[!duplicated(aggregatedDataFrame[c("dev_name", "repo", "count", "email")]), ])
}
...
#return(t(as.matrix(language_df_filtered)))
#}

#graph_languages(languages_df)
```
...
graph_languages <- function(df){
  ...
  # Add traces for each programming language
...
```{r}
# Graph 2: Dev commit count as grouped bar plot
