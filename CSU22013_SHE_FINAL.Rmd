...
```{r}
# Function: Get list of base URLs for top repositories
...
```{r}
# Function: Get list of base URLs for top repositories, and then, create language data frame for repositories
getGitHubData <- function(date_range) {
      ...
      GitHubData <- language_df
}

# Initialise data
GitHubData <- getGitHubData(daterange)
```
...
create_filtered_top_n_language_dataframe <- function(language, language_df, n = 5) {
  ...
  # Filter our repositories with zero% of chosen language
...
top_n_commiters_per_repo <- function(commits_count, n = 5) {
  ...
  for (repo in unique_repos) {
    ...
    result_df <- rbind(result_df, top_devs) # Rbind results to new data frame
  }
  
  row.names(commits_count) <- 1:nrow(commits_count)
...
graph_languages <- function(df){
  ...
  stacked_barplot <- stacked_barplot %>%
    layout(
      title = "Percentage Breakdown of Languages by Repository",
...
plot <- plot_ly(df, 
                ...
                split = ~dev_name) %>%
  layout(
    title = 'Number of Commits per Developer by Repository',
    xaxis = list(title = 'Repository'),
    yaxis = list(title = 'Number of Commits'),
    barmode = 'group',
    bargap = 0.01,        # Adjust the bargap value to increase the width of bars
    bargroupgap = 0.01   # Adjust the bargroupgap value as needed
  )
return(plot)
}
...
```{r}
# Function: Populates dop-down list
getLanguages <- function(date_range, language) {
      
      dropdown_list <- get_list_of_languages(GitHubData)
...
```{r}
# Retrieves data for programming languages percentage stacked bar chart
stackedLanguageBarplot <- function(date_range,language) {
      
  # check if date range has changed - reload data
  if(date_range != daterange)
  {
    GitHubData <- getGitHubData(date_range)
  }
  
  # Get top 5 repos for language
  language_df = create_filtered_top_n_language_dataframe(language, GitHubData)
      
  return(graph_languages(language_df)) 
...
dev_info_table <- function(date_range,language) {
  ...
  # Create data frame for every developers info from top 5 repos for language
...
groupedCommitsBarplot <- function(date_range,language) {
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(selectInput(
      "date_range_dropdown",
      "Sort by GitHub Date Range:",
      choices = c("Daily", "Weekly", "Monthly")
    ),
    selectInput(
      "language_dropdown",
      "Sort by Programming Language:",
      choices = getLanguages(daterange, language)
    )),
    mainPanel(
      textOutput("selected_options1"),
      textOutput("selected_options2"),
      plotlyOutput("stackedBarplot"),
      dataTableOutput("table"),
      plotlyOutput("groupedBarplot"),
    )
  )
))
...
server <- shinyServer(function(input, output) {
  ...
  output$selected_options1 <- renderText({
      paste("You selected:", paste(input$date_range_dropdown, collapse = ", "))
  })
  
  output$selected_options2 <- renderText({
    paste("You selected:", paste(input$language_dropdown, collapse = ", "))
  })
  
  # Display stacked language bar plot
  output$stackedBarplot <- renderPlotly({
    stackedLanguageBarplot(input$date_range_dropdown, input$language_dropdown)
  })
  
  # Display developer info table 
  output$table <- renderDT({
    dev_info_table(input$date_range_dropdown, input$language_dropdown)
  })

  # Display grouped commits bar plot
  output$groupedBarplot <- renderPlotly({
    groupedCommitsBarplot(input$date_range_dropdown, input$language_dropdown)
  })
