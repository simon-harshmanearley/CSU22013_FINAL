...
server <- shinyServer(function(input, output) {
 ...
 # graph1_data <- graph_languages()
#  graph2_data <- generate_graph3_data()
#  graph3_data <- generate_graph4_data()

  output$stackedBarplot <- renderPlotly({

    graph_languages(language_df)
  })

  output$table <- renderDT({

      datatable(commits_count[, -which(names(commits_count) == "count")],options = list(scrollY = "300px"))
  })

  output$groupedBarplot <- renderPlotly({

    graph_commits(commits_count)
