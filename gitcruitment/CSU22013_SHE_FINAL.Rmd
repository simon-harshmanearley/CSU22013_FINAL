...
Libraries, Hosting, Tokens, Globals
...
API_PAT <<-
  c(Authorization = paste("Bearer", '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'))
PAGE_LIMITS <<- 5
...
API_1_create_urls <- function(date_range) {
  ...
  url <-
    mapply(create_url, repo_author, repo_name, SIMPLIFY = FALSE)
...
create_language_dataframe <- function() {
  ...
  for (url in GIT_URLS) {
    ...
    url <-
      paste0(url, "/languages")         # Append /languages to base URL
  ...
  language_table <-
    language_table[, colSums(language_table != 0) > 0]
...
create_filtered_top_n_language_dataframe <-
  function(language, language_df, n = 5) {
    ...
    filtered_repositories <-
      language_df[language_df[, language_index] > 0,]
...
create_commits_dataframe <- function() {
  ...
  for (url in GIT_URLS_FILTERED) {
    ...
    while (page <= PAGE_LIMITS) {
      ...
      df <- data.frame(
        repo = c(strsplit(unlist(
          lapply(commit_data, function (x)
            x$commit$tree$url)
        ), "/")[[1]][6]),
        dev_name = c(unlist(
          lapply(commit_data, function (x)
...
count_commits_by_DevRepo <- function(commits_df) {
  ...
  aggregatedDataFrame <-
    aggregatedDataFrame[!duplicated(aggregatedDataFrame[c("dev_name", "repo", "count", "email")]),]
  
  aggregatedDataFrame <-
    aggregatedDataFrame[order(aggregatedDataFrame$repo,-aggregatedDataFrame$count), ]
...
top_n_commiters_per_repo <- function(commits_count, n = 5) {
  ...
  for (repo in unique_repos) {
    repo_data <-
      commits_count[commits_count$repo == repo,]  # Get rows for each repo
    top_devs <-
      repo_data[order(-repo_data$count),][1:min(dim(repo_data)[1], n),] # Order by count and get top n devs
    result_df <-
      rbind(result_df, top_devs) # Add results to new data frame
  }
...
graph_languages <- function(df) {
  ...
  cols_to_remove <-
    colnames(df)[apply(df, 2, function(col)
      all(col == 0))]
...
language_event <- function(input, output, language) {
  ...
  GIT_URLS_FILTERED <<-
    get_urls_from_list_of_repo_names(language_df_filtered)
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(# Language drop-down menu
        "language_dropdown",
